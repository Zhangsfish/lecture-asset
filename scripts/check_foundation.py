...
REQUIRED = [
    'README.md', 'AGENTS.md', 'STATUS.md', 'LICENSE',
    'docs/PRODUCT_DECISIONS.md', 'docs/IMAGE_POLICY.md', 'docs/SPEC.md',
...
def main() -> int:
    ...
    if isinstance(schema, dict) and isinstance(example, dict):
        ...
        expected_files = set(expected_images + ['README.md', 'lecture.md'])
        if len(paths) != len(set(paths)) or set(paths) != expected_files:
            errors.append('Example file list mismatch.')

        policy = example.get('image_policy', {})
        expected_policy = {
            'codec': 'jpeg',
            'jpeg_quality': 0.9,
        ...
        if policy != expected_policy:
            errors.append('Example image policy does not match frozen v0.1 policy.')
